...
procedure PP_initialize(modelpath : string);
begin
  pp_model := callocN(sizeof(PPModelData), 'PP model data');

  pp_model^.modelpath := IncludeTrailingPathDelimiter(modelpath);

  pp_model^.conductivity_liq := callocN(sizeof(ListBase), 'conductivity_liq');
  pp_model^.conductivity_vap := callocN(sizeof(ListBase), 'conductivity_vap');
  pp_model^.density_liq := callocN(sizeof(ListBase), 'density_liq');
  pp_model^.density_vap := callocN(sizeof(ListBase), 'density_vap');
  pp_model^.cp_gas := callocN(sizeof(ListBase), 'cp_gas');
  pp_model^.enthalpy_vap := callocN(sizeof(ListBase), 'enthalpy_vap');
  pp_model^.surface_tension := callocN(sizeof(ListBase), 'surface_tension');
  pp_model^.vapor_pressure := callocN(sizeof(ListBase), 'vapor_pressure');
  pp_model^.viscosity_liq := callocN(sizeof(ListBase), 'viscosity_liq');
  pp_model^.viscosity_vap := callocN(sizeof(ListBase), 'viscosity_vap');
