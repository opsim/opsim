...
ListBase * scanimage(struct ImBuf * ibuf, int dir)
{
...
void filterdraw(uint * ldest, uint * lsrce, int zero, int half, int step)
{
	uchar * src, * dst;
	int count;
	double weight, add;
...
void filterimage(struct ImBuf * ibuf, struct ImBuf * cbuf, ListBase * listarray, int dir)
{
	int step, pixels, lines, nextline, x, y, pos, drawboth;
	uint * irect, * crect;
...
void _intel_scale(struct ImBuf * ibuf, ListBase * listarray, int dir)
{
	int step, lines, nextline, x, y, pos, col;
	uint * irect, * trect;
	int start, end;
	Edge * left, * right;
	struct ImBuf * tbuf;
