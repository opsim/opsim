...
int Mat4Invert(inverse, mat)
float inverse[][4], mat[][4];
{
	int i, j, k;
	double temp;
	float tempmat[4][4];
...
void Mat4InvertSimp(inverse, mat)
float inverse[][4], mat[][4];
{
...
void Mat4Adj(out,in)	/* out = ADJ(in) */
float out[][4],in[][4];
{
...
void Mat4InvGG(out,in)	/* van Graphic Gems I, out= INV(in)  */
float out[][4],in[][4];
{
	int i, j;
	float det, det4x4();
...
void Mat4MulMat43(m1, m3, m2)	/* m1 en m3 zijn mat4, m2 is mat3 */
float *m1, *m2, *m3;
{
...
void Mat4MulMat34(m1, m3, m2)	/* m1 en m2 zijn mat4, m3 is mat3 */
float *m1, *m2, *m3;
{
...
void Mat4CpyMat4(m1,m2)
float *m1,*m2;
{
	bcopy(m2, m1, 64);
}

void Mat4SwapMat4(m1,m2)
float *m1,*m2;
{
	float t;
	int i;
...
void Mat4CpyMat3nc(m1,m2)	/* no clear */
float m1[][4];
float m2[][3];
{
	m1[0][0]= m2[0][0];
...
void Mat3MulSerie(answ, m1, m2, m3, m4, m5, m6, m7, m8)
float *answ, *m1, *m2, *m3, *m4, *m5, *m6, *m7, *m8;
{
	float temp[3][3];
...
void Mat4MulSerie(answ, m1, m2, m3, m4, m5, m6, m7, m8)
float *answ, *m1, *m2, *m3, *m4, *m5, *m6, *m7, *m8;
{
	float temp[4][4];
...
void VecMat4MulVecfl(in, mat, vec)
float *in, mat[][4];
float *vec;
{
	float x,y;
...
void Mat3TransMulVecfl(mat,vec)
float mat[][3];
float *vec;
{
	float x,y;
...
void Mat3ToQuat(wmat, q)		/* uit Sig.Proc.85 pag 253 */
float wmat[][3], *q;
{
...
void i_ortho(left, right, bottom, top, near, far, matrix)
...
void i_window(float left, float right, float bottom, float top, float near, float far, float mat[][4])
{
...
float dist;
float azimuth, incidence, twist, Vm[][4];
{
...
void VecAddf(v, v1,v2)
register float *v, *v1,*v2;
{
...
void CalcNormShort(v1,v2,v3,n) /* is ook uitprodukt */
short *v1,*v2,*v3;
float *n;
{
	float n1[3],n2[3];
