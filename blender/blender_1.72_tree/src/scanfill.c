...
EditEdge *eed;
{
	...
	if(ed) insertlinkbefore((ListBase *)&(sc->first), ed, eed);
	else addtail((ListBase *)&(sc->first),eed);
...
void scanfill(PolyFill *pf)
{
	...
	float miny,addfac,x;
	int a,b,verts, teller=0, maxvlak, totvlak;
		...
		while(sc->first) {	/* zolang er edges zijn */
			...
			if(ed2==0) {
				...
				if(test) {
					...
					insertlinkbefore((ListBase *)&(sc->first), ed2, ed3);
					ed3->v2->f= 1;
					ed3->f= 2;
					ed3->v1->h++; 
					ed3->v2->h++;
					...
					if(sc1) {	/* ed3 bestaat al: verwijderen*/
						...
						while(ed3) {
							if( (ed3->v1==v1 && ed3->v2==v3) || (ed3->v1==v3 && ed3->v2==v1) ) {
								remlink((ListBase *)&(sc1->first),ed3);
...
int edgefill(int mode)  /* DE HOOFD FILL ROUTINE */
{
	/*
...
*/
	...
	if(poly>1) {
		...
		polycache= pc= callocN(sizeof(short)*poly, "polycache");
		pf= pflist;
		for(a=0; a<poly; a++, pf++) {
			for(c=a+1;c<poly;c++) {
