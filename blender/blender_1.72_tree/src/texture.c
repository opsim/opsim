...
void open_plugin_tex(PluginTex *pit)
{
	...
	/* no dlclose! multiple opened plugins... */
	/* if(pit->handle) dlclose(pit->handle); */
	/* pit->handle= 0; */

	/* open the needed object */
	pit->handle= dlopen(pit->name, (G.f & G_DEBUG)?RTLD_LAZY:RTLD_NOW);
	if(pit->handle==0 || test_dlerr(pit->name, pit->name)) return;

	/* find the address of the version function */
	version= dlsym(pit->handle, "plugin_tex_getversion");
	...
	if (pit->version==2 || pit->version==3) {
		...
		info_func= dlsym(pit->handle, "plugin_getinfo");
		if (!test_dlerr(pit->name, "plugin_getinfo")) {

			info_func(info);

			pit->doit= info->tex_doit;
			pit->callback= info->callback;
...
void free_texture(Tex *tex)
{
	...
	if(tex->env) free_envmap(tex->env);
}
...
Tex *copy_texture(Tex *tex)
{
	...
	if(texn->env) texn->env= copy_envmap(texn->env);
	...
	if(tex->type==TEX_PLUGIN) {
		...
		if(tex->env) {
			...
			if(R.flag & R_RENDERING) {
				if(tex->env->stype==ENV_ANIM) free_envmapdata(tex->env);
			}
		}
	}
}
...
int multitex(Tex *tex, float *texvec, float *dxt, float *dyt)
{
	...
	case TEX_PLUGIN:
		...
		break;
	case TEX_ENVMAP:
		return envmaptex(tex, texvec, dxt, dyt);
		break;
	}
}
		...
		if(R.mat->mtex[tex_nr]) {
			...
			if(tex->nor && (rgbnor & TEX_NOR)==0) {
				...
				if(rgbnor & TEX_RGB) {
					...
					float co= 0.5*fcos(Tin-0.5);
					float si= 0.5*fsin(Tin-0.5);
					float f1, f2;
