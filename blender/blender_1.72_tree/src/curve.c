...
Nurb *duplicateNurb(Nurb *nu)
{
	...
	if(nu->bezt) {
		...
		if(nu->knotsu) {
			len= KNOTSU(nu);
			if(len) {
				new->knotsu= mallocN(len*sizeof(float), "duplicateNurb4");
				memcpy(new->knotsu, nu->knotsu, sizeof(float)*len);
			}
		}
		if(nu->pntsv>1 && nu->knotsv) {
			len= KNOTSV(nu);
			if(len) {
				new->knotsv= mallocN(len*sizeof(float), "duplicateNurb5");
				memcpy(new->knotsv, nu->knotsv, sizeof(float)*len);
			}
		}
	}
	return new;
...
void makeknots(Nurb *nu, short uv, short type)	/* 0: uniform, 1: endpoints, 2: bezier */
{
	if( (nu->type & 7)==CU_NURBS ) {
		if(uv & 1) {
			if(nu->knotsu) freeN(nu->knotsu);
			if(nu->pntsu>1) {
				nu->knotsu= callocN(4+sizeof(float)*KNOTSU(nu), "makeknots");
		...
		if(uv & 2) {
			if(nu->knotsv) freeN(nu->knotsv);
			if(nu->pntsv>1) {
				nu->knotsv= callocN(4+sizeof(float)*KNOTSV(nu), "makeknots");
...
/* data  moet 3*4*resolu*resolv lang zijn en op nul staan */
{
	...
	len= nu->pntsu*nu->pntsv;
	if(len==0) return;
	sum= (float *)callocN(sizeof(float)*len, "makeNurbfaces1");
	...
	basisu= (float *)mallocN(sizeof(float)*KNOTSU(nu), "makeNurbfaces3");
	...
	len= KNOTSV(nu);
	basisv= (float *)mallocN(sizeof(float)*len*resolv, "makeNurbfaces3");
	jstart= (int *)mallocN(sizeof(float)*resolv, "makeNurbfaces4");
	jend= (int *)mallocN(sizeof(float)*resolv, "makeNurbfaces5");
...
float *data;	    /* moet 3*4*pntsu*resolu lang zijn en op nul staan */
{
	...
	len= nu->pntsu;
	if(len==0) return;
	sum= (float *)callocN(sizeof(float)*len, "makeNurbcurve1");
	...
	basisu= (float *)mallocN(sizeof(float)*(nu->orderu+nu->pntsu), "makeNurbcurve3");
...
/* data moet dim*4*pntsu*resolu lang zijn en op nul staan */
{
	...
	ustep= (uend-ustart)/(resolu-1+(nu->flagu & 1));
	basisu= (float *)mallocN(sizeof(float)*KNOTSU(nu), "makeNurbcurve3");
...
void make_orco_surf(Curve *cu)
{
	...
	data= cu->orco= callocN(3*sizeof(float)*tot, "make_orco");
			...
			if(cu->flag & CU_UV_ORCO) {
				for(b=0; b<nu->resolu; b++) {
					for(a=0; a<nu->resolv; a++) {
						...
						if(nu->resolv<2) data[0]= 0.0;
						else data[0]= -1.0 + 2.0*((float)a)/(nu->resolv-1);
						
						if(nu->resolu<2) data[1]= 0.0;
						else data[1]= -1.0 + 2.0*((float)b)/(nu->resolu-1);
...
void makebevelcurve(Object *ob, ListBase *disp)
{
	...
	if(cu->bevobj && cu->bevobj!=ob) {
			...
			if(bevcu->ext1==0.0 && bevcu->ext2==0.0) {
				...
				while(dl) {
					if ELEM(dl->type, DL_POLY, DL_SEGM) {
						...
						*dlnew= *dl;
						dlnew->verts= mallocN(3*sizeof(float)*dl->parts*dl->nr, "makebevelcurve1");
		...
		dl->verts= mallocN(2*3*sizeof(float), "makebevelcurve2");
		addtail(disp, dl);
		dl->type= DL_SEGM;
		dl->parts= 1;
		dl->nr= 2;
		...
		dl->verts= mallocN(nr*3*sizeof(float), "makebevelcurve3");
		addtail(disp, dl);
		dl->type= DL_SEGM;
		dl->parts= 1;
		dl->nr= nr;
...
void calc_bevel_sin_cos(x1, y1, x2, y2, sina, cosa)
float x1, y1, x2, y2, *sina, *cosa;
{
	float t01, t02, x3, y3;
	...
	while(nu) {
		if(nu->pntsu>1) {
			...
			if((nu->type & 7)==CU_POLY) {
				...
				data= mallocN(3*sizeof(float)*(nu->resolu+1), "makeBevelList2");
				data_a= callocN(sizeof(float)*(nu->resolu+1), "data_a");
				...
				if(nu->pntsv==1) {
					...
					data= callocN(4*sizeof(float)*len, "makeBevelList4");    /* moet op nul staan */
