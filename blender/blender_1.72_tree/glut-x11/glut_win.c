...
__glutCreateWindow(GLUTwindow * parent,
  int x, int y, int width, int height, int popup)
{
  GLUTwindow *window;
...
glutCreateWindow(const char *title)
{
  ...
  window = __glutCreateWindow(NULL,
    ...
    __glutInitWidth, __glutInitHeight, False);
  win = window->win;
  ...
  zeldaiscool=XAllocClassHint(); /*NEW!!*/
  zeldaiscool->res_name=strdup(title); /*NEW!!*/ 
  zeldaiscool->res_class=strdup(title); /*NEW!!*/ 
  ...
  /* Only put WM_COMMAND property on first window. */
    ...
    &__glutSizeHints, wmHints, zeldaiscool); /*CHANGED!!*/
  XFree(wmHints);
  free(zeldaiscool->res_name);/*NEW!!*/
  free(zeldaiscool->res_class);/*NEW!!*/
  XFree(zeldaiscool); /*NEW!!*/  
...
int APIENTRY
glutCreatePopupWindow(const char *title)
/*
 * Desc: create a glut window with save under enabled
 *	 the window is not handled by the WM, so no hints
 */
{
  GLUTwindow *window;
  Window win;
  ...
  window = __glutCreateWindow(NULL,
    __glutSizeHints.x, __glutSizeHints.y,
    __glutInitWidth, __glutInitHeight, True);
  win = window->win;
...
{
  __glutChangeWindowEventMask(KeyPressMask | KeyReleaseMask,
