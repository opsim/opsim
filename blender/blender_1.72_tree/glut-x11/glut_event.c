...
interruptibleXNextEvent(Display * dpy, XEvent * event)
{
  ...
  for (;;) {
	...
	/* ZR - glut hung during rendering when the window 
		was stowed... we added the timeout to the select
		call and this return (and a fake event in 
		processEventsandTimeouts) and that seemd to fix it */
...
processEventsAndTimeouts(void)
{
	...
	do {
		...
		XEvent event, ahead;
		GLUTwindow *window;
		static int last_press_time=0;	/* X patch, we don't want a silly repeat */
		...
		if (gotEvent) {
			...
			case KeyRelease:
				...
				if (window->keyboard && window->special) {
					KeySym ks;
					
					if (XEventsQueued(__glutDisplay, QueuedAfterReading)) {
						XPeekEvent(__glutDisplay, &ahead);
						if(ahead.type==KeyPress && ((XKeyEvent *)&event)->time==((XKeyEvent *)&ahead)->time) {
							ks = XLookupKeysym((XKeyEvent *) & event, 0);
							if(ks==XLookupKeysym((XKeyEvent *) & ahead, 0)) break;
						}
					}
...
skip:
			...
			case VisibilityNotify:
				...
				break;
			case ClientMessage:
				/* ZR - Blender doesn't quit for just any peon */
