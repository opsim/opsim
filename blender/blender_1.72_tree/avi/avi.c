...
AviError avi_open_movie (char *name, AviMovie *movie) {
	unsigned int temp, fcca, size, fcctmp;
	char tmp[5];
	...
	movie->header = (AviMainHeader *) mallocN (sizeof (AviMainHeader), "movieheader");
	...
	movie->streams = (AviStreamRec *) mallocN (sizeof(AviStreamRec) * movie->header->Streams, "moviestreams");

	for (temp=0; temp < movie->header->Streams; temp++) {
		movie->streams[temp].sf = NULL;
		...
		if (movie->streams[temp].sh.Type == FCC("vids")) {
			...
			} else if (fcca == FCC ("JMF "))
				movie->streams[temp].format = AVI_FORMAT_JMF;
		...
		if (movie->streams[temp].sh.Type == FCC("vids")) {
			if (movie->streams[temp].sf_size == sizeof(AviBitmapInfoHeader)-8) {
				...
				movie->streams[temp].sf= mallocN(sizeof(AviBitmapInfoHeader), "streamformat");
...
void *avi_read_frame (AviMovie *movie, AviFormat format, int frame, int stream) {
	...
	temp = GET_FCC(movie->fp);
	buffer = mallocN (temp,"readbuffer");
...
AviError avi_open_compress (char *name, AviMovie *movie, int streams, ...) {
	...
	movie->offset_table = (long *) mallocN ((1+streams*2) * sizeof (long),"offsettable");
	...
	for (i=0; i < movie->header->Streams; i++) {
		...
		if (movie->streams[i].sh.Type == FCC("vids")) {	
			if (movie->streams[i].format == AVI_FORMAT_MJPEG) {
				movie->streams[i].sf = mallocN (sizeof(AviBitmapInfoHeader) 
										+ sizeof(AviMJPEGUnknown),"moviestreamformatL");
				movie->streams[i].sf_size = sizeof(AviBitmapInfoHeader) + sizeof(AviMJPEGUnknown);
			} else {
				movie->streams[i].sf = mallocN (sizeof(AviBitmapInfoHeader),  "moviestreamformatS");
...
AviError avi_write_frame (AviMovie *movie, int frame_num, ...) {
	...
	if (frame_num+1 > movie->index_entries) {
		temp = (AviIndexEntry *) mallocN ((frame_num+1) * 
