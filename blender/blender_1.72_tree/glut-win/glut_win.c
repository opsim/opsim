...
/* CENTRY */
int APIENTRY
glutGetActiveWindow(void)
{
  int i;
  HWND focused= GetFocus();
	
  for (i = 0; i < __glutWindowListSize; i++) {
	if (__glutWindowList[i] && __glutWindowList[i]->win == focused)
		return i+1;
  }
  return 0;
}
...
/* The bogus windows should be resolved, instead of
 * the errors being commented out. DUHH! */
...
#if defined(WIN32)
  WNDCLASS wc;
  if (!GetClassInfo(GetModuleHandle(NULL), "Blender", &wc))
...
glutCreateWindow(const char *title)
{
  ...
  if (firstWindow) firstWin = window;
  win = window->win;
