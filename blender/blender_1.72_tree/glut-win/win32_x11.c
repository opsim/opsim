...
short forceborderless= 0; /* To allow borderless on Windows */
...
{
	static int first=1; /* So we know which window this is */
	
	if (first) {
		first=0;
		if (forceborderless) {
			return CreateWindow("Blender", "Blender", 
					WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_MAXIMIZE | WS_POPUP,
					x, y, width, height, parent, NULL,
					GetModuleHandle(NULL), 0);
		} else {
			return CreateWindow("Blender", "Blender", 
					WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_MAXIMIZE | WS_OVERLAPPEDWINDOW,
					x, y, width, height, parent, NULL,
					GetModuleHandle(NULL), 0);			
		}
	} else
		return CreateWindow("Blender", "Render", 
